...
def analyze_emoji_sentiment_distribution(input_file, output_dir):
    ...
    print("SUMMARY")
    print("="*80)
    print(f"✓ Analyzed {len(df):,} total posts")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Analyze emoji distribution by sentiment class")
    parser.add_argument("--input", required=True, help="Path to input parquet file")
    parser.add_argument("--output-dir", required=True, help="Output directory for results")
    args = parser.parse_args()
    
    analyze_emoji_sentiment_distribution(args.input, args.output_dir)
